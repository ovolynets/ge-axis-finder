...
//  Float_t peak_en = 0.24; // 238keV
  Float_t peak_en = 1.333; // 1.33MeV of Co
...
//  ch1->Add("/remote/pclg-23/volynets/Work/mage/SII_gammas/new/top/1.33/root_occ_imp0.0_0.35/*");
...
//  Int_t Phi = 20;
  ...
  TH1F *hOccMC = new TH1F(Form("hOccMC_%d",Phi),"Occupancy in segments; #phi [deg.];Occupancy",6,0,360.);
...
//  for (int iEntry=0;iEntry<100000;iEntry++)
  {
    ...
    for (int iHit=0; iHit<nhits; iHit++)
    {
      ...
      hitSegID = GetSegID(Phi, z[iHit], phi[iHit]);
//      hitSegID = int(phi[iHit]/(60));
...
//  gPad->SetTopMargin(0.01);
  ...
  c1->RedrawAxis();
  c1->SaveAs(Form("Top_Occ_MC_demonstr_%d_%4.2f.eps",Phi,peak_en));
  c1->SaveAs(Form("Top_Occ_MC_demonstr_%d_%4.2f.root",Phi,peak_en));
//   After closing file, the histogram are deleted from memory
//   for (int iAngle=0;iAngle<fNangles;iAngle++)
//   {  delete hOccMC[iAngle]; }
