...
#define TMCOccSideSelector_cxx
// The selector skeleton was automatically created by ROOT.
// Here is the information from ROOT:

// The class definition in TMCOccSideSelector.h has been generated automatically
...
//
// Root > T->Process("TMCOccSideSelector.C")
// Root > T->Process("TMCOccSideSelector.C","some options")
// Root > T->Process("TMCOccSideSelector.C+")
//

#include "TMCOccSideSelector.h"
#include <TH2.h>
#include <TStyle.h>
...
Int_t TMCOccSideSelector::GetSegID_side(
    const Double_t rad_angle, // deg: Angle at which source is positioned
    const Double_t zz,        // cm
...
void TMCOccSideSelector::Begin(TTree * /*tree*/)
{
...
void TMCOccSideSelector::SlaveBegin(TTree * /*tree*/)
{
  ...
  fPeakEnergy   = mypar->fPeakEnergy;   // Peak energy
  fCurrentAngle = mypar->fCurrentAngle; // Current axis angle being tested
  fSourceAngle  = mypar->fSourceAngle;  // Source angle

//    For tests: also fill the histograms to benchmark
//   for ( int iLayer = 0; iLayer < 3; iLayer++)
//   {
//     hist[iLayer] = new TH1F(Form("hOccMC_%d_%04.1f", iLayer, fCurrentAngle), "hOccMC", 6, 0, 360.);
//     fOutput->Add(hist[iLayer]);
//   }

  // It is very important to add every object to be extracted later
  // to the fOutput object
  fOutPars = new MyOutputParameters(Form("outpars_%04.1f",fCurrentAngle));
  fOutput->Add(fOutPars);

}

Bool_t TMCOccSideSelector::Process(Long64_t entry)
{
  // The Process() function is called for each entry in the tree (or possibly
  // keyed object in the case of PROOF) to be processed. The entry argument
  // specifies which entry in the currently loaded tree is to be processed.
  // It can be passed to either TMCOccSideSelector::GetEntry() or TBranch::GetEntry()
  ...
  // Please see TMCOccTopSelector for detailed description
  fChain->GetEntry(entry);

  Double_t SegEnergy[18];   // Segment energies
  Double_t CoreEnergy = 0;  // Core energy
...
//     hist[lID]->AddBinContent(firedSegID%6+1);

    // For side case only one angle is processed at the same time
    // (in contradiction to angle loop in TMCOccTopSelector, so
    // always 0-th index is used for the file occ[0][firedSegID] )
    fOutPars->occ[0][firedSegID]++;
  }
  return kTRUE;
}

void TMCOccSideSelector::SlaveTerminate()
{
...
void TMCOccSideSelector::Terminate()
{
...
//   for ( int iLayer = 0; iLayer < 3; iLayer++)
//   {
//     hist[iLayer] = dynamic_cast<TH1F*>(fOutput->FindObject(Form("hOccMC_%d_%04.1f", iLayer, fCurrentAngle)));
//   }


  // Only the output parameters object is extracted after internal merging
  fOutPars = dynamic_cast<MyOutputParameters*>(fOutput->FindObject(Form("outpars_%04.1f",fCurrentAngle)));
