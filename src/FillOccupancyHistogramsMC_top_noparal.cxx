...
void TAxisFinder::FillOccupancyHistogramsMC_top_noparal()
{
  /**
  ...
  for (int iEntry=0;iEntry<nentries;iEntry++)
  {
    ...
    for (int iLoop=0;iLoop<fAngles_N;iLoop++)
    {
      int iAngle=fAngles[iLoop];
      Double_t SegEnergy[18];
      Double_t CoreEnergy = 0;
      for (int iSeg=0;iSeg<18;iSeg++)
      {  SegEnergy[iSeg] = 0;  }

      for (int iHit=0;iHit<nhits;iHit++)
      {
        int hitSegID = GetSegID_top(iAngle, z[iHit], phi[iHit]);
